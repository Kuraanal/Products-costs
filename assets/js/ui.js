...
function renderDashboard() {
  ...
  const sellableProducts = appState.products.filter(function (p) {
    return !p.isAvailableAsIngredient || p.sellingPrice > 0;
  });
  const ingredientOnlyProducts = appState.products.filter(function (p) {
    return p.isAvailableAsIngredient && p.sellingPrice <= 0;
  });
  ...
  let warningsHTML = "";
  const productsWithNoIngredients = sellableProducts.filter(function (p) {
    return p.ingredients.length === 0;
  });
  if (productsWithNoIngredients.length > 0) {
    warningsHTML =
      '<div class="warning-banner">⚠️ ' +
      productsWithNoIngredients.length +
      " product(s) have no ingredients in their recipe.</div>";
  }

  let rankingHTML = "";
  if (productStats.length > 0) {
    rankingHTML =
      '<div class="section-title">Profit Margin Ranking</div><div class="ranking-list">';
  ...
  viewContainer.innerHTML =
    warningsHTML +
    '<div class="stat-grid">' +
    '<div class="card stat-card"><div class="stat-icon">🧂</div><div class="stat-value">' +
    totalIngredients +
    '</div><div class="stat-label">Ingredients</div></div>' +
    '<div class="card stat-card"><div class="stat-icon">🍜</div><div class="stat-value">' +
    sellableProducts.length +
    '</div><div class="stat-label">Products</div></div>' +
    '<div class="card stat-card"><div class="stat-icon">📦</div><div class="stat-value">' +
    ingredientOnlyProducts.length +
    '</div><div class="stat-label">Sub-Recipes</div></div>' +
    '<div class="card stat-card"><div class="stat-icon">💰</div><div class="stat-value">' +
    (productStats.length > 0 ? productStats[0].margin.toFixed(1) + "%" : "—") +
    '</div><div class="stat-label">Best Margin</div></div>' +
    "</div>" +
    '<div class="card">' +
    (rankingHTML ||
      '<div class="empty-state"><div class="empty-icon">📊</div><p>No products yet. Add some to see rankings!</p></div>') +
    "</div>";
}
...
function renderIngredientList() {
  ...
  var filtered = appState.ingredients.filter(function (ing) {
    ...
    return (
      ing.name.toLowerCase().indexOf(ingredientSearchQuery.toLowerCase()) >= 0
    );
  });
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    tableRows +=
      '<tr data-id="' + ing.id + '" style="cursor:pointer;">' +
      "<td>" +
      escapeHTML(ing.name) +
      "</td>" +
      '<td class="hide-mobile"><span class="unit-badge">' +
      ing.unit +
      "</span></td>" +
      "<td>" +
  ...
  viewContainer.innerHTML =
    ...
    " ingredient(s)</span>" +
    '<input class="input-field search-input" id="ingredient-search" type="text" placeholder="\uD83D\uDD0D Search ingredients..." value="' +
    escapeHTML(ingredientSearchQuery) +
    '">' +
    "</div>" +
    (filtered.length > 0
      ? '<div class="card"><div class="table-responsive"><table class="ingredient-table"><thead><tr>' +
        '<th>Name</th><th class="hide-mobile">Unit</th><th>Price</th><th>Amount</th><th class="hide-mobile">Cost/Unit</th>' +
        "</tr></thead><tbody>" +
        tableRows +
        "</tbody></table></div></div>"
  ...
  // Event: Click row to edit
  var rows = viewContainer.querySelectorAll('.ingredient-table tbody tr[data-id]');
  rows.forEach(function (row) {
    row.addEventListener('click', function (e) {
      // Open modal on click
      openIngredientModal(row.dataset.id);
    });
  });
}
...
function openIngredientModal(editId) {
  ...
  const body =
    '<div class="form-group"><label>Name</label><input class="input-field" id="ing-name" value="' +
    (existing ? escapeHTML(existing.name) : "") +
    '" placeholder="e.g., Sugar"></div>' +
    '<div class="form-group"><label>Unit</label><select class="input-field" id="ing-unit">' +
    '<option value="g"' +
    (existing && existing.unit === "g" ? " selected" : "") +
    ">Grams (g)</option>" +
    '<option value="ml"' +
    (existing && existing.unit === "ml" ? " selected" : "") +
    ">Millilitres (ml)</option>" +
    '<option value="piece"' +
    (existing && existing.unit === "piece" ? " selected" : "") +
    ">Piece</option>" +
    "</select></div>" +
    '<div class="form-row">' +
    '<div class="form-group"><label>Purchase Price (' +
    appState.settings.currency +
    ')</label><input class="input-field" id="ing-price" type="number" step="0.01" min="0" value="' +
    (existing ? existing.purchasePrice : "") +
    '" placeholder="0"></div>' +
    '<div class="form-group"><label>Purchase Amount</label><input class="input-field" id="ing-amount" type="number" step="0.01" min="0.01" value="' +
    (existing ? existing.purchaseAmount : "") +
    '" placeholder="1"></div>' +
    "</div>";
  ...
  if (isEdit) {
    document.getElementById("modal-delete").addEventListener("click", function () {
      var result = deleteIngredient(editId);
      if (result.error) {
...
function renderProductList(tab) {
  ...
  headerActions.innerHTML =
    '<button class="btn btn-primary" id="btn-add-product">+ New Product</button>';

  // Split products into two groups
  var sellableProducts = appState.products.filter(function (p) {
    return !p.isAvailableAsIngredient || p.sellingPrice > 0;
  });
  var ingredientOnlyProducts = appState.products.filter(function (p) {
    return p.isAvailableAsIngredient && p.sellingPrice <= 0;
  });

  var activeList =
    currentProductTab === "sellable"
      ? sellableProducts
      : ingredientOnlyProducts;
  ...
  for (let i = 0; i < activeList.length; i++) {
    ...
    cardsHTML +=
      '<div class="card product-card" data-id="' +
      p.id +
      '">' +
      ...
      '</span><span class="stat-label">Yield Cost</span></div>' +
      (currentProductTab === "sellable"
        ? '<div class="stat"><span class="stat-value">' +
          formatCurrency(profit.yieldProfit) +
          '</span><span class="stat-label">Yield Profit</span></div>'
        : "") +
      '<div class="stat hide-mobile"><span class="stat-value">' +
      formatCurrency(profit.unitCost) +
      '</span><span class="stat-label">Unit Cost</span></div>' +
      (currentProductTab === "sellable"
        ? '<div class="stat hide-mobile"><span class="stat-value">' +
          formatCurrency(profit.unitProfit) +
          '</span><span class="stat-label">Unit Profit</span></div>'
        : "") +
      (currentProductTab === "sellable"
        ? '<div class="stat"><span class="stat-value ' +
          marginClass +
          ...
          '%</span><span class="stat-label">Margin</span></div>'
        : "") +
      "</div>" +
      "</div>";
  }
  ...
  viewContainer.innerHTML =
    ...
    (currentProductTab === "sellable" ? " active" : "") +
    '" data-product-tab="sellable">Products (' +
    sellableProducts.length +
    ")</button>" +
    '<button class="settings-tab' +
    (currentProductTab === "ingredients" ? " active" : "") +
    '" data-product-tab="ingredients">Sub-Recipes (' +
    ingredientOnlyProducts.length +
    ")</button>" +
    "</div>" +
    (activeList.length > 0
...
function openProductModal(editId) {
  ...
  const body =
    '<div class="form-group"><label>Product Name</label><input class="input-field" id="prod-name" value="' +
    (existing ? escapeHTML(existing.name) : "") +
    '" placeholder="e.g., Boba Milk Tea"></div>' +
    '<div class="form-row">' +
    '<div class="form-group"><label>Yield Amount</label><input class="input-field" id="prod-yield" type="number" step="0.01" min="0.01" value="' +
    (existing ? existing.yieldAmount : "1") +
    '"></div>' +
    '<div class="form-group"><label>Yield Unit</label><select class="input-field" id="prod-yield-unit">' +
    '<option value="piece"' +
    (existing && existing.yieldUnit === "piece" ? " selected" : "") +
    ">Piece</option>" +
    '<option value="g"' +
    (existing && existing.yieldUnit === "g" ? " selected" : "") +
    ">Grams (g)</option>" +
    '<option value="ml"' +
    (existing && existing.yieldUnit === "ml" ? " selected" : "") +
    ">Millilitres (ml)</option>" +
    "</select></div>" +
    "</div>" +
    '<div class="form-row">' +
    '<div class="form-group"><label>Selling Price per Unit (' +
    appState.settings.currency +
    ')</label><input class="input-field" id="prod-price" type="number" step="0.01" min="0" value="' +
    (existing ? existing.sellingPrice : "") +
    '" placeholder="0"></div>' +
    '<div class="form-group"><label>Labor Cost Override (optional)</label><input class="input-field" id="prod-labor" type="number" step="0.01" min="0" value="' +
    (existing && existing.laborCostOverride !== null
      ? existing.laborCostOverride
      : "") +
    '" placeholder="Use global"></div>' +
    "</div>";
...
function renderProductDetail(productId) {
  ...
  for (let i = 0; i < product.ingredients.length; i++) {
    ...
    recipeHTML +=
      ...
      (getProductById(item.id)
        ? ' <span class="badge badge-purple">product</span>'
        : "") +
      "</span>" +
      '<span class="item-amount">' +
      formatDisplayUnit(item.amount, compUnit) +
      "</span>" +
      '<span class="item-cost">' +
      formatCurrency(itemCost) +
      "</span>" +
      '<button class="btn-icon btn-remove-recipe-item" data-index="' +
      i +
      '" title="Remove">✕</button>' +
      "</div>";
  }
  ...
  viewContainer.innerHTML =
    ...
    '<input type="checkbox" id="chk-use-as-ingredient"' +
    (product.isAvailableAsIngredient ? " checked" : "") +
    ">" +
    ...
    '<div class="card summary-stat"><div class="stat-value">' +
    product.yieldAmount +
    " " +
    product.yieldUnit +
    '</div><div class="stat-label">Yield</div></div>' +
    '<div class="card summary-stat"><div class="stat-value">' +
    formatCurrency(profit.totalCost) +
    '</div><div class="stat-label">Total Cost</div></div>' +
    '<div class="card summary-stat"><div class="stat-value">' +
    formatCurrency(profit.unitCost) +
    '</div><div class="stat-label">Unit Cost</div></div>' +
    '<div class="card summary-stat"><div class="stat-value">' +
    formatCurrency(product.sellingPrice) +
    '</div><div class="stat-label">Selling Price</div></div>' +
    '<div class="card summary-stat"><div class="stat-value ' +
    marginClass +
    '">' +
    formatCurrency(profit.unitProfit) +
    '</div><div class="stat-label">Unit Profit</div></div>' +
    '<div class="card summary-stat"><div class="stat-value ' +
    marginClass +
    '">' +
    profit.margin.toFixed(1) +
    '%</div><div class="stat-label">Margin</div></div>' +
    "</div>" +
    // Recipe section
    ...
    (product.ingredients.length > 0
      ? '<div class="recipe-list">' +
        recipeHTML +
        "</div>" +
        '<div class="recipe-total-footer"><span>Total Ingredients Cost</span><span>' +
        formatCurrency(profit.ingredientsCost) +
        "</span></div>" +
        '<div class="recipe-total-footer" style="margin-top:var(--space-sm);"><span>Labor Cost</span><span>' +
        formatCurrency(profit.laborCost) +
        "</span></div>"
  ...
  // --- Events ---
  document
    .getElementById("btn-back-products")
  ...
  document
    .getElementById("btn-edit-product")
    .addEventListener("click", function () {
      openProductModal(productId);
    });

  document
    .getElementById("btn-delete-product")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("chk-use-as-ingredient")
    .addEventListener("change", function () {
      ...
      if (!this.checked) {
        for (var i = 0; i < appState.products.length; i++) {
          ...
          if (
            p.ingredients.some(function (item) {
      ...
      showToast(
        this.checked
          ? "Now available as ingredient."
          : "Removed from ingredients list.",
        "success",
      );
    });

  document
    .getElementById("btn-add-recipe-item")
    .addEventListener("click", function () {
...
function openRecipeItemModal(productId) {
      ...
      .forEach(function (el) {
        el.addEventListener("click", function () {
          ...
          selectedDiv.innerHTML =
            ...
            " (" +
            el.dataset.unit +
            ')</span> <button class="btn-icon btn-clear-selection" style="font-size:0.8rem;" title="Clear">✕</button>';
  ...
  document.getElementById("modal-save").addEventListener("click", function () {
    ...
    var existingIdx = product.ingredients.findIndex(function (item) {
...
function renderSettings() {
  ...
  viewContainer.innerHTML =
    ...
    "<h3>Global Costs</h3>" +
    '<div class="form-group"><label>Default Labor Cost (' +
    appState.settings.currency +
    ")</label>" +
    '<input class="input-field" id="settings-labor" type="number" step="0.01" min="0" value="' +
    appState.settings.globalLaborCost +
    '">' +
    "</div>" +
    '<div class="form-group"><label>Currency</label>' +
    '<input class="input-field" id="settings-currency" value="' +
    escapeHTML(appState.settings.currency) +
    '" placeholder="TWD">' +
    "</div>" +
  ...
  tabs.forEach(function (tab) {
    tab.addEventListener("click", function () {
      ...
      viewContainer.querySelectorAll(".tab-content").forEach(function (tc) {
  ...
  document
    ...
    .addEventListener("click", function () {
      appState.settings.globalLaborCost =
        parseFloat(document.getElementById("settings-labor").value) || 0;
      appState.settings.currency =
        document.getElementById("settings-currency").value.trim() || "TWD";
      saveData();
